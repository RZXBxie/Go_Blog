...
// GetAccessToken 从请求头获取accessToken
func GetAccessToken(c *gin.Context) string {
	token := c.Request.Header.Get("x-access-token")
	return token
}

// GetRefreshToken 从Cookie获取refreshToken
...
func GetClaims(c *gin.Context) (*request.JwtCustomClaims, error) {
	...
	if err != nil {
		global.Log.Error("Failed to Failed to retrieve JWT parsing information from Gin's Context. Please check if the request header contains 'x-access-token' and if the claims structure is correct.", zap.Error(err))
	}
	return claims, err
}
...
func GetRefreshClaims(c *gin.Context) (*request.JwtCustomRefreshClaims, error) {
	...
	if err != nil {
		global.Log.Error("Failed to Failed to retrieve JWT parsing information from Gin's Context. Please check if the request header contains 'x-refresh-token' and if the claims structure is correct.", zap.Error(err))
	}
	return claims, err
}
